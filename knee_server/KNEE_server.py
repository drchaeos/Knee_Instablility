...
OUTPUT_DIR = "./out"
column_DIR = './columns/knee_stress.csv'
...
app.mount("/static", StaticFiles(directory="/app/knee/static"), name="static")
app.mount("/images", StaticFiles(directory="/app/knee/images"), name="images")
...
# Jinja2 템플릿 디렉토리
templates = Jinja2Templates(directory="/app/knee/templates")


# 업로드 및 변환 함수
@app.post("/upload_and_analyze_acl/")
async def upload_and_analyze(request: Request, image: UploadFile = File(...)):
    ...
    result = subprocess.run(["python", "ACL_inference.py"], capture_output=True, text=True)

    # ACL_inference.py 스크립트가 'ACL_result.csv'라는 이름의 파일을 출력으로 생성한다고 가정
    dataname = '[ACL]'
    csv_path = os.path.join(OUTPUT_DIR, f'{dataname}_result_{image_name}.csv')

    with open(csv_path, mode='r') as csv_file:
        reader = csv.DictReader(csv_file)
        data = [row for row in reader]

    analyzed_image_name = "ACL_result_" + image.filename
    return {"data": data, "image_path": f"/out/{analyzed_image_name}"}

# 업로드 및 변환 함수
@app.post("/upload_and_analyze_pcl/")
async def upload_and_analyze(request: Request, image: UploadFile = File(...)):
    # 업로드한 이미지를 특정 위치에 저장
    print(image.filename)
    uploaded_image_path = os.path.join(UPLOAD_DIR, image.filename)
    ...
    # 분석 스크립트를 사용하여 이미지 분석
    result = subprocess.run(["python", "PCL_inference.py"], capture_output=True, text=True)
    ...
    analyzed_image_name = "PCL_result_" + image.filename
    return {"data": data, "image_path": f"/out/{analyzed_image_name}"}
...
async def serve_index(request: Request):
    ...
    index_path = os.path.join(current_directory, "knee", "templates", "signup.html")
...
async def serve_index():
    current_directory = os.path.dirname(os.path.abspath(__file__))
    index_path = os.path.join(current_directory, "knee", "templates", "explanation.html")
    with open(index_path, "r", encoding="utf-8") as f:
        content = f.read()
    return HTMLResponse(content=content)
...
async def serve_index():
    current_directory = os.path.dirname(os.path.abspath(__file__))
    index_path = os.path.join(current_directory, "knee", "templates", "acl_index.html")
    with open(index_path, "r", encoding="utf-8") as f:
        content = f.read()
    return HTMLResponse(content=content)
...
async def serve_index():
    current_directory = os.path.dirname(os.path.abspath(__file__))
    index_path = os.path.join(current_directory, "knee", "templates", "pcl_index.html")
    with open(index_path, "r", encoding="utf-8") as f:
        content = f.read()
    return HTMLResponse(content=content)
